...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existiingDocument = await ctx.db.get(args.id);

    if (!existiingDocument) throw new Error("Not found");

    if (existiingDocument.userId !== userId) throw new Error("Unauthorized");
